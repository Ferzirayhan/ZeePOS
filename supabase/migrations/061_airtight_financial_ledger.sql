-- ====================================================================
-- Migrasi 061: Airtight Financial Ledger, Shift Serialization, and Role Hardening
-- ====================================================================
-- 1. Drop overload lama pay_receivable_atomic (4 argumen).
-- 2. Hapus bypass GUC zeepos.internal_mutation. Kunci kolom total_hutang & is_active
--    secara mutlak di level PostgreSQL Trigger & Column Permissions.
-- 3. Serialisasi Cash Shift: Kunci row cash_shifts FOR UPDATE saat checkout tunai
--    dan cicilan piutang tunai untuk mencegah race condition dengan closing shift.
-- 4. Kunci cancel_transaction_atomic dari transaksi ambigu legacy.
-- 5. create_transaction_atomic mengembalikan full record transaksi & items resmi.
-- ====================================================================

-- 1. Drop overload lama tanpa idempotency key dan drop versi lama 5 argumen untuk reset default parameters
DROP FUNCTION IF EXISTS public.pay_receivable_atomic(integer, numeric, text, text);
DROP FUNCTION IF EXISTS public.pay_receivable_atomic(integer, numeric, text, text, text);

-- 2. Proteksi Kolom Finansial Customer Tanpa GUC Bypass
...
BEGIN
  ...
  ELSIF TG_OP = 'UPDATE' THEN
    -- Tolak jika diubah langsung dari sesi client authenticated/anon
...
-- Cabut hak UPDATE langsung pada kolom total_hutang dan is_active untuk authenticated & anon
REVOKE UPDATE (total_hutang, is_active) ON public.customers FROM authenticated, anon, PUBLIC;

-- 3. Update cancel_transaction_atomic dengan proteksi ambiguous_legacy
...
BEGIN
  ...
  -- Tolak transaksi yang memiliki konversi unit stok ambigu legacy
  IF EXISTS (
    ...
    RAISE EXCEPTION 'Transaksi lama memiliki konversi stok ambigu; pembatalan otomatis ditolak dan memerlukan rekonsiliasi manual';
  END IF;
...
-- 4. pay_receivable_atomic dengan Shift Locking (Anti Race Condition)
...
BEGIN
  ...
  -- SERIALISASI SHIFT: Jika pembayaran tunai, kunci row shift kasir yang aktif
...
-- 5. create_transaction_atomic dengan Shift Locking & Full Transaction Snapshot Return
...
BEGIN
  ...
  -- SERIALISASI SHIFT: Jika transaksi tunai, kunci baris cash_shifts milik kasir ini
