-- ====================================================================
-- Migrasi 064: Idempotency Wajib Checkout, Shift Gate Non-Tunai, dan
--              Rekonsiliasi Konfirmasi Pembayaran
-- ====================================================================
-- 1. create_transaction_atomic: idempotency key TIDAK lagi optional.
--    Tanpa key, RPC menolak — dua submit tanpa key tidak bisa membukukan
--    transaksi duplikat (menutup kelas bug overload piutang lama).
-- 2. pay_receivable_atomic: gate shift aktif diperluas ke SEMUA metode
--    (sebelumnya hanya tunai) sehingga cicilan non-tunai juga terekonsiliasi.
-- 3. confirm_transaction_payment: konfirmasi QRIS/transfer pending wajib
--    shift aktif milik konfirmator (atau admin) + kunci row shift, jadi
--    dana masuk tidak lagi lolos di luar shift mana pun.
-- 4. close_cash_shift: tidak menutup shift dengan transaksi pending
--    yang belum dikonfirmasi; rekonsiliasi tunai pakai paid_at supaya
--    konfirmasi setelah close masuk shift berikutnya (bukan dobel).
-- 5. Konfirmasi transaksi menulis cash_shift_id pada transaction_refunds
--    tidak berubah — hanya confirm yang mendapat gate baru.
-- ====================================================================

-- --------------------------------------------------------------------
-- 1. create_transaction_atomic — idempotency key WAJIB
--    Body identik dengan 062; hanya validasi key yang ditambahkan.
-- --------------------------------------------------------------------
...
-- --------------------------------------------------------------------
-- 2. pay_receivable_atomic — gate shift untuk SEMUA metode (tunai maupun
--    non-tunai) supaya cicilan QRIS/transfer juga terekonsiliasi ke shift.
--    Body identik dengan 062; hanya blok gate yang diperluas.
-- --------------------------------------------------------------------
...
BEGIN
  ...
  -- SERIALISASI SHIFT: pembayaran BARU (tunai maupun non-tunai) wajib memiliki shift
  -- kasir aktif dan mengunci barisnya supaya diserialisasi terhadap close_cash_shift.
  -- Ditempatkan SETELAH lookup idempotensi agar retry atas pembayaran yang sudah
  -- commit tetap berhasil meski shift sudah ditutup.
  IF NOT EXISTS (
    SELECT 1
    FROM public.cash_shifts
    ...
    RAISE EXCEPTION 'Tidak ada shift kasir yang aktif. Buka shift terlebih dahulu sebelum menerima pembayaran cicilan.';
  END IF;
...
-- --------------------------------------------------------------------
-- 3. confirm_transaction_payment — konfirmasi pending QRIS/transfer wajib
--    shift aktif milik konfirmator (atau admin), kunci row shift, dan
--    idempoten terhadap dana yang sudah dikonfirmasi.
-- --------------------------------------------------------------------
...
BEGIN
  ...
  -- SERIALISASI SHIFT: konfirmasi dana masuk wajib dilakukan di dalam shift aktif
  -- milik konfirmator (admin boleh di luar shift miliknya sendiri jika tidak ada
  -- shift terbuka untuk transaksi tersebut). Tanpa ini, QRIS/transfer yang
  -- dikonfirmasi setelah shift ditutup tidak masuk rekonsiliasi shift mana pun.
...
-- --------------------------------------------------------------------
-- 4. close_cash_shift — rekonsiliasi tidak boleh kehilangan uang:
--    (a) tolak menutup bila masih ada transaksi pending milik shift ini
--        (kasir harus konfirmasi/batalkan dulu), dan
--    (b) hitung tunai berdasarkan paid_at sehingga transaksi yang
--        dikonfirmasi SETELAH close masuk shift berikutnya, bukan
--        dihitung dobel di shift lama.
-- --------------------------------------------------------------------
...
BEGIN
  ...
  -- (a) Tolak menutup bila masih ada transaksi pending yang belum dikonfirmasi
  --     milik shift ini — mencegah dana masuk lolos tanpa rekonsiliasi.
  SELECT COUNT(*) INTO v_pending_count
  FROM public.transactions
  WHERE tenant_id = v_tenant_id
    AND kasir_id = v_shift.kasir_id
    AND metode_bayar IN ('qris', 'transfer')
    AND payment_status = 'menunggu_konfirmasi'
    AND created_at >= v_shift.opened_at;

  IF v_pending_count > 0 THEN
    RAISE EXCEPTION 'Masih ada % transaksi QRIS/transfer yang belum dikonfirmasi. Konfirmasi atau batalkan terlebih dahulu sebelum closing shift.', v_pending_count;
  END IF;

  -- 1. Penjualan tunai gross (berdasarkan paid_at: kapan dana benar-benar masuk).
  --    Transaksi yang dikonfirmasi SETELAH shift ditutup jatuh ke shift berikutnya.
  ...
  -- 4. Penerimaan non-tunai riil (QRIS & Transfer) yang sudah dibayar. Atribusi ke
  --    pemilik shift berdasarkan confirmed_by: konfirmasi oleh admin/staf lain di
  --    dalam shift mereka tetap masuk rekonsiliasi shift yang benar (tidak hilang
  --    di antara shift kasir pembuat transaksi dan shift konfirmator).
  SELECT COALESCE(SUM(total), 0)
  ...
  WHERE tenant_id = v_tenant_id
    ...
    AND metode_bayar IN ('qris', 'transfer')
    AND (
      confirmed_by = v_shift.kasir_id
      OR (confirmed_by IS NULL AND kasir_id = v_shift.kasir_id)
    );
  ...
  PERFORM public.insert_audit_log(
    ...
    jsonb_build_object(
      ...
      'pending_ditolak', v_pending_count,
      'selisih', v_selisih
    )
  );
...
-- --------------------------------------------------------------------
-- 5. Grant eksplisit (house style 062): fungsi yang di-re-emit tetap
--    executable oleh klien yang sah, trigger/revoke defensif dipertahankan.
-- --------------------------------------------------------------------
